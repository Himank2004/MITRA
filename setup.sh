...
start_ml_api() {
  ...
  if check_port 5001; then
    echo -e "${YELLOW}Port 5001 is in use. Stopping existing process(es) before starting ML API...${NC}"
    kill_on_port 5001
  fi
  ...
  # Validate API keys (agent uses a hierarchy spanning both providers)
  if [ -z "${GOOGLE_API_KEY:-}" ]; then
    echo -e "${YELLOW}\u26a0 GOOGLE_API_KEY is not set. Add it to ML_Backend/.env${NC}"
  fi
  if [ -z "${GROQ_API_KEY:-}" ]; then
    echo -e "${YELLOW}\u26a0 GROQ_API_KEY is not set. Add it to ML_Backend/.env${NC}"
  fi
  echo -e "${GREEN}\u2713 Model hierarchy: gemini-flash-2.5 → flash-lite → llama-70b → llama-8b → gpt-oss-120b${NC}"
...
echo -e "  ${YELLOW}* ML API:${NC} Running on http://localhost:5001"
