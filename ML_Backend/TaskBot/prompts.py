...
create_task_prompt = PromptTemplate(
    ...
    template="""You are a therapy task creation assistant. Output ONLY valid JSON. No explanations, no preamble, no markdown prose, no reasoning steps, no "Task Creation Logic" sections — ONLY the raw JSON object.
...
{{
    ...
    "existing_tasks": [
        {{
            ...
            "task_type": "The type of the task (e.g., 'discrete', 'slider', 'checkmark', 'journal').",
            "reason_for_task_creation": "The reason or message explaining why the task was created.",
            "description": "A detailed description of the task.",
            "completed": "The current progress of the task.",
            "total_count": "Number of counts to complete the task (discrete tasks only).",
            "difficulty": "The difficulty level of the task (e.g., 'easy', 'medium', 'hard').",
            "recurring": "Number of hours before the task repeats, 0 by default."
        }}
    ]
}}
```
...
- Gradually increase difficulty based on user progress.
- Tailor tasks to the reason and align with therapy goals.
- Frame tasks positively.
- Output ONLY a JSON object — no other text before or after.
