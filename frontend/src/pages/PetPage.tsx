...
const ML_API_URL = process.env.REACT_APP_API_URL || "http://localhost:5001";
const BACKEND_URL =
...
const PetPage: React.FC = () => {
    ...
    const urlConversationId =
        new URLSearchParams(location.search).get("conversationId") || null;
    ...
    useEffect(() => {
        fetchConversations().then(() => {
            setConversations((prev) => {
                ...
                // Prefer URL-provided conversationId, then keep current, then first
                setActiveId(
                    (cur) => urlConversationId ?? cur ?? prev[0].conversationId,
                );
                return prev;
            });
        });
    ...
    // Covers: initial page load, navigating here from the Home chat card, and
    // any programmatic activeId set.  handleSelectConv also calls this, but
    // loadMessagesFromDB short-circuits immediately if messages are already present.
    useEffect(() => {
        if (!activeId) return;
        const conv = conversations.find((c) => c.conversationId === activeId);
        if (conv) loadMessagesFromDB(conv);
    ...
    const handleSelectConv = (conv: LocalConv) => {
        ...
        // message load is handled by the activeId effect above
    };
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        const isFirstExchange = currentConv?.messages.length === 0;
        ...
        try {
            ...
            // Auto-title after first exchange (user msg + bot reply).
            // Only fires when the DB title is still the default "New Conversation"
            // so manual renames and reflection titles are never overwritten.
            if (isFirstExchange && botText) {
                setConversations((prev) => {
                    const current = prev.find(
                        (c) => c.conversationId === capturedConvId,
                    );
                    const currentLabel = current?.label ?? "";
                    const isDefault =
                        currentLabel === "New Conversation" ||
                        currentLabel === "Conversation" ||
                        currentLabel === "";
                    if (isDefault) {
                        const autoTitle = generateAutoTitle(message, botText);
                        // Update local label immediately inside the setter
                        const updated = prev.map((c) =>
                            c.conversationId === capturedConvId
                                ? { ...c, label: autoTitle }
                                : c,
                        );
                        // Fire the DB persist outside React's batch (side-effect)
                        renameConversationInDB(capturedConvId, autoTitle);
                        return updated;
                    }
                    return prev;
                });
    ...
    // ── Auto-title: build a concise title from first user + bot message ────
    const generateAutoTitle = (userMsg: string, botMsg: string): string => {
        const userSnip = userMsg.replace(/\s+/g, " ").trim().slice(0, 30);
        const botRaw = botMsg
            .replace(/[#*`_]/g, "")
            .replace(/\s+/g, " ")
            .trim();
        const sentEnd = botRaw.search(/[.!?]/);
        const botSnip =
            sentEnd > 2 && sentEnd <= 28
                ? botRaw.slice(0, sentEnd)
                : botRaw.slice(0, 24);
        const u = userSnip + (userMsg.length > 30 ? "\u2026" : "");
        if (!botSnip) return u;
        return `${u} \u00b7 ${botSnip}${botRaw.length > 24 && sentEnd < 0 ? "\u2026" : ""}`;
    };

    // ── Persist title to DB ──────────────────────────────────────────────────
    const renameConversationInDB = useCallback(
    ...
    // ── Start inline edit ────────────────────────────────────────────────────
    const startEdit = (c: LocalConv, e: React.MouseEvent) => {
        e.stopPropagation();
        setEditingConvId(c.conversationId);
        setEditTitle(c.label);
        setTimeout(() => editInputRef.current?.select(), 0);
    };

    const commitEdit = async (convId: string) => {
        const title = editTitle.trim();
        setEditingConvId(null);
        if (!title) return;
        updateConv(convId, () => ({ label: title }));
        await renameConversationInDB(convId, title);
    };
    ...
    return (
        <div className="flex flex-col min-h-screen bg-[#F9FAFB]">
            ...
            <div className="flex flex-1 max-w-5xl mx-auto w-full px-4 pb-10 gap-6 items-start">
                {/* ─── Sidebar ─── */}
                <aside className="hidden md:flex flex-col w-64 shrink-0 gap-3 sticky top-4 max-h-[calc(100vh-2rem)] overflow-y-auto">
                    {/* Conversation list */}
                                ...
                                >
                                    {editingConvId === c.conversationId ? (
                                        <input
                                            ...
                                            onBlur={() =>
                                                commitEdit(c.conversationId)
                                            }
                                            onClick={(e) => e.stopPropagation()}
                                            className="flex-1 min-w-0 bg-white border border-[#C66408] rounded px-2 py-0.5 text-sm text-[#1F2937] focus:outline-none focus:ring-1 focus:ring-[#C66408]"
                                        />
                                    ) : (
                                        <>
                                            ...
                                            <button
                                                ...
                                                className="opacity-0 group-hover:opacity-100 text-gray-300 hover:text-[#C66408] transition text-sm leading-none focus:outline-none px-0.5"
                                                title="Rename"
                                            ...
                                            <button
                                                ...
                                                className="opacity-0 group-hover:opacity-100 ml-0.5 text-gray-300 hover:text-red-400 transition text-lg leading-none focus:outline-none"
                                                title="Delete"
                            ...
                            >
                                <div
                                    className={`max-w-[78%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                                        m.from === "user"
                                            ? "bg-[#FFEEDB] text-[#1F2937] rounded-br-sm whitespace-pre-wrap"
                                            : m.from === "bot"
                                              ? "bg-[#F3F4F6] text-[#1F2937] rounded-bl-sm"
                                              : "bg-yellow-50 text-yellow-800 text-xs italic whitespace-pre-wrap"
                                    }`}
                                >
                                    {m.from === "bot" ? (
                                        m.text ? (
